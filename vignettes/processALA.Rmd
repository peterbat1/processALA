...
To complete the installation of **processALA**, ensure that the package **devtools** is installed from CRAN, then enter the following command at the prompt in the _**R**_ console:

```{r eval=FALSE}
devtools::install_github("peterbat1/processALA")
```
...
The package **processALA** adds another level of refinement. It highly automates key steps in the retrieval and processing of ALA data by utilising a few key functions in the ALA4R package and adds functions to assist in data cleaning.
...
The ALA API provides a number of functions which allow you to interrogate APNI and APC to resolve plant names to a matching accepted name from APC. Two functions in AL4R, _search_names()_ and _species_info()_ are available to determine the status of a name and provide details associated with it. The **processALA** function _checkTaxonName()_ uses them to check a name you have supplied and return a subset information useful for further processing of records.
...
The ALA4R function _occurrences()_ returns a list with two elements _meta_ (metadata about the data providers whose records are present in the download) and _data_ which is the table of occurrence data. Both are standard _**R**_ data.frames. _fetchALAdata()_ saves each in the specified output folder using the following naming convention: _meta_ is saved as "Genus_speciesEpithet_metadata.csv" and _data_ is saved as "Genus_specificEpithet.csv".
...
As a final point, note that image and living specimen data are not currently extracted and saved. These data are however present in the raw data csv file and you may therefore use an _**R**_-script or spreadsheet program (Excel, or LibreOffice Calc on the Linux PCs) to extract them if you need access to them.
