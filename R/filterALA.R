                          ...
                          trace = FALSE)
{
  ...
  for (thisTaxon in taxa)
  {
    ...
    inFilename <- gsub("//", "/", paste0(baseDataPath, "/",thisTaxon, "/", this_Taxon,"_", recType, ".csv"))
    outFilename <- gsub("//", "/", paste0(baseDataPath, "/",thisTaxon, "/", this_Taxon,"_", recType, "_filtered.csv"))
    ...
    if (file.exists(inFilename))
    {
      ...
      if (trace)
        cat("      Final number of records: ", nrow(theRecords),"\n")
